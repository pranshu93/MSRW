...
class FastRNNBonsai_NoZ:
    def __init__(self, C, F, D, S, lW, lT, lV,
                 sW, sT, sV, W=None, T=None, V=None):

        self.dataDimension = F
        self.projectionDimension = F # since no Z
        if (C > 2):
            self.numClasses = C
        elif (C == 2):
            self.numClasses = 1
    ...
    def bonsaiGraph(self, X):
        ...
        # Bonsai
        X_ = tf.reshape(X, [self.dataDimension, -1])
...
def getArgs():
    ...
    parser.add_argument('-out', type=str, default='out.txt', help='Output filename')
    ...
    parser.add_argument('-noZ', default=False, action='store_true', help='If enabled, projection matrix Z is excluded')
...
if args.noZ:
    fastrnnbonsaiObj = FastRNNBonsai_NoZ(num_classes, hidden_dim, depth, sigma,
                                     regW, regT, regV, sparW, sparT, sparV)
else:
...
bonsai_model_size = fastrnnbonsaiObj.getModelSize()
if args.noZ:
    projectionDimension = None
    regZ = None
    sparZ = None
results_list = [args.ggnl, args.gunl, args.ur, args.wr, args.w, args.sp, args.lr, args.bs, args.hs, args.ot,
       args.ml, args.fn, sigma, depth, projectionDimension, regZ, regT, regW, regV, sparZ, sparW, sparV, sparT, bonsai_model_size, max_acc]
