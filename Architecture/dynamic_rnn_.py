...
def main():
    def getArgs():
        ...
        parser.add_argument('-unl', type=str, default="quantTanh" , help='tanh/sigmoid/relu/quantSigm/quantTanh')
    ...
    def forward_iter(data, labels, data_seqlen, index, code):
        ...
        else: return(sess.run([accuracy,logits], feed_dict={X: batchx, Y: batchy, seqlen: batchz, learning_rate: lr}))
    ...
    for i in range(num_epochs):
        ...
        acc,_ = forward_iter(train_data,train_labels,train_seqlen,slice(0,train_data.__len__()),False)
    ...
    os.system("python " + os.path.abspath('../Scripts/quantizeFastModels.py') + " -dir " + modelloc)
    ...
    def predict(points,lbls):
        ...
        for i in range(points.shape[0]):
            ...
            for t in range(seq_max_len):
                ...
                h_ = np.array(nonlin("s",pre+qB_h*I,q*I)/(q),dtype=fpt)
                z = np.array(nonlin("t",pre+qB_g*I,q*I)/(q),dtype=fpt)
...
#print ("Shape: ", v.shape)
#print (formatp(v))
    predict(train_cuts,train_cuts_lbls)
    #saver.restore(sess, modelloc + "/bestmodel.ckpt")
    ans,_ = forward_iter(train_data,train_labels,train_seqlen,slice(0,train_data.__len__()),False)
    print(ans)
''''''
