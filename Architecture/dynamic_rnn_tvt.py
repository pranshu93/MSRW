...
def main():
    ...
    # New TVT logic - Dhrubo
    train_cuts = np.load(os.path.join(fileloc, args.type+ "_train.npy"))
    train_cuts_lbls = np.load(os.path.join(fileloc, args.type + "_train_lbls.npy"))

    val_cuts = np.load(os.path.join(fileloc, args.type + "_val.npy"))
    val_cuts_lbls = np.load(os.path.join(fileloc, args.type + "_val_lbls.npy"))
    ...
    #print(mean,std)
    train_cuts_n = []; test_cuts_n = []; val_cuts_n = [];
    ...
    if args.q15:
        ...
        [val_cuts_n.append(q15_to_float(((np.array(val_cuts[i]) - mean) / std).astype(int)).tolist()) for i in range(val_cuts.shape[0])]
    else:
        ...
        [val_cuts_n.append(((np.array(val_cuts[i])-mean)/std).tolist()) for i in range(val_cuts.shape[0])]
    ...
    val_data, val_labels, val_seqlen = process(val_cuts_n,val_cuts_lbls)
