...
def generate_trainstring(v):
    ...
    res_str=[]
    for i in range(1,6):
        res_str.append(call_str + " -fn " + str(i) + " -type " + args.type + " -q15 True -bat slurm -out $outname")
...
with open(args.type + '_big_q15.sh', 'w') as f:
    print('outname=`echo $0 | sed "s/.sh/.out/g"`', file=f)
with open(args.type + '_big_q15.sh', 'a') as f:
    print(*results, sep = "\n", file=f)
